...
let videoResult = (track, video, matchType) =>
  resolve({
    "artist": getTrackArtist(track),
    "song": track##track##name,
...
let sanitize = (str) => {
  let result =
    str |> Js.String.toLowerCase |> Js.String.replaceByRe([%bs.re "/[^a-z0-9\\s]/g"], "");
...
let searchForKeyword = (track, video, keyword) => {
  let searchString = (sanitize @@ getTrackArtist(track)) ++ sanitize(track##track##name);
  let sanitizedTitle = sanitize(video##snippet##title);
  ...
  if (Js.String.includes(keyword, searchString)) {
    ...
    switch (Js.String.match(regex, sanitizedTitle)) {
    | Some(_) => true
    | _ => false
    }
  }
...
let classifyVideo = (track, video) : match('a) => {
  let matchType =
    searchForKeyword(track, video, "acoustic") ?
      `Acoustic :
      searchForAnyKeywords(track, video, [|"studio", "sessions?"|]) ?
        `LiveInStudio :
        searchForKeyword(track, video, "live") ?
          `Live :
          searchForKeyword(track, video, "audio") ?
            `AudioOnly :
            searchForAllKeywords(track, video, [|"lyrics?", "official"|]) ?
              `OfficialLyricVideo :
              searchForKeyword(track, video, "lyrics?") ?
                `LyricVideo :
                searchForKeyword(track, video, "official") ?
                  `OfficialVideo :
                  video##snippet##channelTitle
                  |> Js.String.toLowerCase
                  |> Js.String.includes("vevo") ?
                    `OfficialVideo : `Unknown;
  {video, matchType}
};

let findVideoOfType = (matchType, matches) =>
  matches |> Js.Array.find((switch_) => switch_.matchType === matchType);
...
let matchTrack = (ytHelper, track) => {
  ...
  |> then_((data) => resolveBestMatch(track, data##result##items))
};
